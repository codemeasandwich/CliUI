 ...
 *
 * @param {object} screen - Blessed screen instance with screen.lines and screen.rows/cols
 * @param {object} opts
 ...
 * @param {object} [opts.topCutout] - Top cutout info (for geometry only)
...
function renderCutoutBody(screen, opts) {
  ...
  var sbGlyphs = [];
  if (hasScrollbar) {
    sbGlyphs = scrollbar.computeScrollbar(viewport.bodyHeight, totalLines, scrollOffset);
  }
  ...
  // Return metadata for testing — the caller can inspect these
  return {
    viewport: viewport,
    virtualLines: vLines,
