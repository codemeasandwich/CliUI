 ...
 * |---------------|-------------|--------------|--------------|------------------|
 * | top-right     | yi          | xi+CW+1      | xl-1         | xi+1             |
 * | top-left      | yi          | xi           | xl-CW-2      | xl-CW-1          |
 * | bottom-right  | yl-1        | xi+CW+1      | xl-1         | xi+1             |
 * | bottom-left   | yl-1        | xi           | xl-CW-2      | xl-CW-1          |
 *
 * Text sits outside the step, at the opposite end of the border from the corner:
 *   right-side corners: text at left end,  columns [xi+1 .. xi+CW]       (right-aligned, flush against step wall)
 *   left-side corners:  text at right end, columns [xl-CW-1 .. xl-2]     (left-aligned, flush against step wall)
 *
...
function stepGeometry(position, xi, xl, yi, yl, CH, CW) {
  ...
  case 'top-right':
    return {
      ...
      stepLeftCol:  xi + CW + 1,
      stepRightCol: xl - 1,
      textColBase:  xi + 1        // text in [xi+1 .. xi+CW], right-aligned
    };
  case 'top-left':
    return {
      junctionRow:  yi,
      stepLeftCol:  xi,
      stepRightCol: xl - CW - 2,
      textColBase:  xl - CW - 1   // text in [xl-CW-1 .. xl-2], left-aligned
    };
  case 'bottom-right':
    return {
      junctionRow:  yl - 1,
...
function stepJunctionChars(position, charset) {
  ...
  case 'bottom-right':
    // Junction (box bottom row): topRight at stepLeftCol; bottomRight at stepRightCol (unchanged)
    return { leftChar: charset.topRight, rightChar: null };
  case 'bottom-left':
    // Junction (box bottom row): bottomLeft at stepLeftCol (unchanged); topLeft at stepRightCol
    return { leftChar: null, rightChar: charset.topLeft };
  default:
 ...
 *
 * Per §4.3, short lines are padded toward the step wall (flush against it):
 *   - top-right, bottom-right: RIGHT-align (text at left end, flush right against step wall)
 *   - top-left, bottom-left: LEFT-align (text at right end, flush left against step wall)
 *
 * @param {string} position - corner position
 * @returns {'left'|'right'}
 */
