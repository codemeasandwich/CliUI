...
function Diagram(options) {
  ...
  /** @type {Box|null} Lazy-created "re-center" button shown when panned off-center. */
  ...
  /** @type {Map<string,number>|null} User string ID → internal box ID. */
  this._nodeIdMap = null;

  /** @type {Map<number,string>|null} Internal box ID → user string ID. */
  this._nodeReverseMap = null;

  // ── Connection animation state ────────────────────────────────

  /** @type {Map<number,Object>|null} Connector ID → anim state. */
  this._connAnimStates = null;

  // ── Keyboard focus state ──────────────────────────────────────

  /** @type {number|null} Currently focused box ID. */
  this._focusedBoxId = null;

  /** @type {number[]} Ordered list of focusable box IDs. */
  this._focusOrder = [];

  /** @type {number} Focus pulse animation frame counter. */
  this._focusPulseFrame = 0;

  /** @type {ReturnType<typeof setInterval>|null} Focus pulse timer. */
  ...
  this.on('detach', function onDetach() {
    ...
    self._clearConnAnimations();
    self._stopFocusPulse();
  });
...
/*
 * Stub methods — these are overridden by widget-render.js and
 * widget-keyboard.js once those modules are loaded.  The stubs
 * prevent 'not a function' errors if the detach handler fires
 * before the patch modules have been required.
 */
Diagram.prototype._clearConnAnimations = function () {};
Diagram.prototype._stopFocusPulse = function () {};
