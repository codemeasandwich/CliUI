...
/**
 * Synchronise model width/height with the widget's content area.
 *
 * @private
 */
Diagram.prototype._syncModelSize = function _syncModelSize() {
  ...
  /* Expand model to fill the container, but never exceed it — an
   * oversized model produces lines longer than the content area,
   * causing blessed to wrap text and corrupt the diagram. */
  if (this._model.width  < contentWidth)  this._model.width  = contentWidth;
  if (this._model.width  > contentWidth)  this._model.width  = contentWidth;
  if (this._model.height < contentHeight) this._model.height = contentHeight;
  if (this._model.height > contentHeight) this._model.height = contentHeight;
};
...
Diagram.prototype._fullRender = function _fullRender() {
  ...
  /* Snapshot the new frame for future incremental diffs. */
  this._prevFrame = new Frame(text, this._model.width, this._model.height);
...
Diagram.prototype._incrementalRender = function _incrementalRender() {
  ...
  var curr = new Frame(text, this._model.width, this._model.height);
