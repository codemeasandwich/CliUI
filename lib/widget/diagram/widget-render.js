...
var applyStatusStyle      = require('./render-box').applyStatusStyle;
...
Diagram.prototype._fullRender = function _fullRender() {
  ...
  var text = renderModel(this._model, {
    ...
    focusedBoxId: this._focusedBoxId,
    focusPulseFrame: this._focusPulseFrame,
    connAnimStates: this._connAnimStates
  });
  ...
  /* Show or hide the re-center button based on current pan state. */
  this._updateCenterButton();

  if (this.screen) {
    this.screen.render();

    /*
     * Status colour pass — applies foreground colour to border cells
     * of boxes that have a status set.  Must run after screen.render()
     * so that blessed has written cell attributes into screen.lines.
     *
     * Pending status also activates currentWork (animated border dots)
     * to give a visual "in progress" feel.
     */
    if (this.screen.lines && this.lpos && this._model) {
      var xi = this.lpos.xi + this.ileft, yi = this.lpos.yi + this.itop;
      var lines = this.screen.lines, panX = this._panX, panY = this._panY;
      var needsRepaint = false;
      this._model.boxes.forEach(function (box) {
        if (!box.status) return;
        applyStatusStyle(box, lines, xi, yi, panX, panY);
        needsRepaint = true;
      });
      if (needsRepaint) this.screen.render();
    }
  }
};
