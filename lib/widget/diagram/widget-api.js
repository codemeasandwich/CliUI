...
Diagram.prototype.setSource = function setSource(text) {
  ...
  /* New source resets the viewport to the origin. */
  this._panX = 0;
  this._panY = 0;
...
Diagram.prototype.layout = function layoutDiagram(options) {
  ...
  /* Layout fits content to the widget — reset pan to show the result. */
  this._panX = 0;
  this._panY = 0;
 ...
 * Pan is clamped so the viewport origin never goes below (0, 0).
 *
...
Diagram.prototype.pan = function pan(dx, dy) {
  this._panX = Math.max(0, this._panX + dx);
  this._panY = Math.max(0, this._panY + dy);
  this._fullRender();
...
Diagram.prototype.panTo = function panTo(x, y) {
  this._panX = Math.max(0, x);
  this._panY = Math.max(0, y);
  this._fullRender();
  this.emit('pan', { panX: this._panX, panY: this._panY });
};

/**
 * Reset the viewport pan to the origin (0, 0).
 */
Diagram.prototype.resetPan = function resetPan() {
  this._panX = 0;
  this._panY = 0;
  this._fullRender();
  this.emit('pan', { panX: 0, panY: 0 });
};
