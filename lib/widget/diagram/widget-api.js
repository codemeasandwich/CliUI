...
Diagram.prototype.setData = function setData(data) {
  if (typeof data === 'string') {
    ...
    /*
     * Structured data path: convert { nodes, connections } into a
     * model via buildModelFromData, then auto-layout and route.
     * Stores the ID maps so callers can translate between user-defined
     * string IDs and internal numeric box IDs.
     */
    var result = buildModelFromData(data, data.defaultBorder);
    ...
    /* Auto-layout + route, then center viewport. */
    this.layout();

    /* Start per-connector animation timers for styled connectors
     * (animated, snake, dashed, spinner). Must run after layout()
     * because connectors need routed segments for cell paths. */
