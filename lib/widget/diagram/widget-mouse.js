...
/**
 * Convert a blessed mouse event to diagram model coordinates.
 *
 * Subtracts the widget's absolute screen position then adds the
 * current pan offset so the returned coordinates are in model space.
 *
...
Diagram.prototype._mouseToLocal = function _mouseToLocal(mouse) {
  ...
  var lx = mouse.x - oX + this._panX;
  var ly = mouse.y - oY + this._panY;

  /* With negative pan offsets, valid clicks on empty space above/left
   * of the model origin produce negative model coordinates. The
   * hit-test returns EMPTY for out-of-bounds, so this is safe. */
  return { x: lx, y: ly };
};

/**
 * Convert a blessed mouse event to screen-local coordinates
 * (widget-relative, without pan offset).
 *
 * Used by the pan-drag handler to compute cursor delta in screen
 ...
 * @private
 */
Diagram.prototype._mouseToScreen = function _mouseToScreen(mouse) {
  var oX = (this.aleft || 0) + this.ileft;
  var oY = (this.atop  || 0) + this.itop;
  var lx = mouse.x - oX;
  var ly = mouse.y - oY;
