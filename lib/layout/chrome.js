 ...
 *
 * Page-specific layout (separators, dividers, static fixups) can be injected
 * via setLayout() with a raw layout spec, or via setPage(pageName) which
 * internally calls computePageLayout to derive the layout for the current
 * screen dimensions. Combining character handling (U+0300–U+036F) uses the
 * shared isCombining utility from display-width.js.
 */
...
var displayWidth = require('../utils/display-width');
...
module.exports = function createChrome(screen, opts) {
  ...
  el.on('render', function paintChrome() {
    ...
    // space for the gap between ┛ and text content. Uses Array.from to
    // iterate by Unicode code point (not UTF-16 code unit) so combining
    // characters and surrogate pairs are handled correctly. Combining chars
    // attach to the previous cell without advancing the column counter.
    var footerChars = Array.from(footer);
    var footerCol = stepCol + 1;
    for (var fi = 0; fi < footerChars.length; fi++) {
      var fch = footerChars[fi];
      var fcp = fch.codePointAt(0);
      if (displayWidth.isCombining(fcp)) {
        // Combining char — attach to the previous cell's content string
        if (footerCol > stepCol + 1 && lines[botY] && lines[botY][footerCol - 1]) {
          lines[botY][footerCol - 1] = [attr, (lines[botY][footerCol - 1][1] || '') + fch];
        }
        continue;
      }
      if (footerCol > maxX) break;
      writeCell(lines, botY, footerCol, attr, fch);
      footerCol++;
    }
    // Fill remaining columns with spaces to clear any stale content
    for (var xr = footerCol; xr <= maxX; xr++) {
  ...
  return {
    ...
    /**
     * Set the current page by name for separator, divider, and fixup painting.
     * Internally calls computePageLayout to derive the layout specification
     * for the current screen dimensions, then applies it via setLayout.
     * This makes chrome self-contained — callers use setPage('spec') instead
     * of manually wiring computePageLayout → setLayout.
     *
     * @param {string} pageName - 'spec'|'plan'|'run'|'task'|'errors'|'perf'
     */
    setPage: function setPage(pageName) {
      currentLayout = pageCompute.computePageLayout(pageName, screen.cols, screen.rows);
    },
...
function paintLogoLine(lines, row, text, attr, logoWidth) {
  ...
  for (var i = 0; i < chars.length; i++) {
    ...
    // Uses the shared isCombining utility which covers the full Unicode combining
    // range (U+0300–U+036F and beyond). Must process combining chars even after
    // col exceeds logoWidth because the combining char modifies the previous
    // cell's content, not a new cell.
    if (displayWidth.isCombining(cp)) {
