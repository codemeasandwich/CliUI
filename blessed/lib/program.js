...
Program.prototype.enableGpm = function() {
  ...
  this.gpm.on('mousewheel', function(btn, modifier, x, y, dx, dy) {
    var action;
    // Determine scroll action: vertical takes priority, then horizontal
    if (dy !== 0) {
      action = dy > 0 ? 'wheelup' : 'wheeldown';
    } else if (dx !== 0) {
      action = dx > 0 ? 'wheelright' : 'wheelleft';
    } else {
      // No scroll movement
      return;
    }
...
// force: if true, skip the async timer and write immediately (for cleanup/exit)
Program.prototype._twrite = function(data, force) {
  var self = this
    , iterations = 0
    , timer;
  ...
  if (this.tmux) {
    ...
    // Skip this async path if force=true (needed for cleanup during exit).
    if (this.output.bytesWritten === 0 && !force) {
...
Program.prototype.setMouse = function(opt, enable) {
  ...
  if (opt.allMotion != null) {
    ...
    if (this.tmux && this.tmuxVersion >= 2) {
      ...
      // When disabling, force sync write to ensure cleanup before exit
      else this._twrite('\x1b[?1003l', true);
    } else {
