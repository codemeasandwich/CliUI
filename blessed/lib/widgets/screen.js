...
function Screen(options) {
  ...
  // See: https://github.com/chjj/blessed/issues/441
  this._onProgramResize = function() {
    self.alloc();
    self.render();
    (function emit(el) {
      el.emit('resize');
  ...
  this._onProgramFocus = function() {
    self.emit('focus');
  };
  this._onProgramBlur = function() {
    self.emit('blur');
  };
  this._onProgramWarning = function(text) {
    self.emit('warning', text);
  };

  this.program.on('resize', this._onProgramResize);
  this.program.on('focus', this._onProgramFocus);
  this.program.on('blur', this._onProgramBlur);
  this.program.on('warning', this._onProgramWarning);
...
Screen.prototype.destroy = function() {
  ...
  // Remove program listeners to allow screen recreation
  // See: https://github.com/chjj/blessed/issues/441
  if (this._onProgramResize) {
    this.program.removeListener('resize', this._onProgramResize);
    this.program.removeListener('focus', this._onProgramFocus);
    this.program.removeListener('blur', this._onProgramBlur);
    this.program.removeListener('warning', this._onProgramWarning);
    delete this._onProgramResize;
    delete this._onProgramFocus;
    delete this._onProgramBlur;
    delete this._onProgramWarning;
  }
...
Screen.prototype.spawn = function(file, args, options) {
  ...
  var resume = function() {
    ...
    // Switch to alternate buffer and enable mouse BEFORE restoring output.write
    // to prevent mouse escape codes from being printed to the terminal.
    // See: https://github.com/chjj/blessed/issues/307
    ...
    // Restore output.write AFTER mouse is configured
