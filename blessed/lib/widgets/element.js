...
function Element(options) {
  ...
  // Clone options to prevent mutations from affecting the original object (#309)
  ...
  if (!this.style) {
    ...
    this.style.italic = options.italic;
    this.style.transparent = options.transparent;
  }

  // Support opacity option (0.0 to 1.0). If transparent is set without opacity,
  // default to 0.5 for backward compatibility.
  this.style.opacity = options.opacity;
  if (this.style.transparent && this.style.opacity == null) {
  ...
  // Border is handled via getter/setter to allow dynamic updates
...
// Border getter/setter to allow dynamic border updates after initialization
...
Element.prototype.sattr = function(style, fg, bg) {
  ...
  if (typeof italic === 'function') italic = italic(this);
      ...
      if (slash) {
        if (!param) {
          ...
          if (attr == null) {
            ...
            state.pop();
            if (state.length) {
              var prevAttr = program._attr(state[state.length - 1]);
              if (prevAttr != null) out += prevAttr;
            } else {
              out += attr;
            }
          }
...
Element.prototype._align = function(line, width, align) {
  ...
  var cline = line.replace(/\x1b\[[\d;]*m/g, '')
    , len = this.screen.fullUnicode ? unicode.strWidth(cline) : cline.length
    , s = width - len;
  ...
  if (align === 'center') {
    ...
    var leftLen = this.screen.fullUnicode ? unicode.strWidth(cparts[0]) : cparts[0].length;
    var rightLen = this.screen.fullUnicode ? unicode.strWidth(cparts[1]) : cparts[1].length;
    s = Math.max(width - leftLen - rightLen, 0);
    s = Array(s + 1).join(' ');
...
main:
  for (; no < lines.length; no++) {
    ...
    var lineDisplayWidth = this.screen.fullUnicode ? unicode.strWidth(line) : line.length;
    while (lineDisplayWidth > width) {
  ...
  out.mwidth = out.reduce(function(current, line) {
    ...
    var w = self.screen.fullUnicode ? unicode.strWidth(line) : line.length;
...
Element.prototype._getLeft = function(get) {
  if (!this.parent) return (this.position.left || 0);
  var parent = get ? this.parent._getPos() : this.parent
    , left = this.position.left || 0
    , expr;
...
Element.prototype._getRight = function(get) {
  if (!this.parent) return (this.position.right || 0);
  var parent = get ? this.parent._getPos() : this.parent
    , right;
...
Element.prototype._getTop = function(get) {
  if (!this.parent) return (this.position.top || 0);
  var parent = get ? this.parent._getPos() : this.parent
    , top = this.position.top || 0
    , expr;
...
Element.prototype._getBottom = function(get) {
  if (!this.parent) return (this.position.bottom || 0);
  var parent = get ? this.parent._getPos() : this.parent
    , bottom;
...
Element.prototype.render = function() {
  ...
  // Calculate blend/alpha for opacity support
  var blend = this.style.opacity != null && this.style.opacity < 1;
  var alpha = blend ? 1 - this.style.opacity : 0.5;
