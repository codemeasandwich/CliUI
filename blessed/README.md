...
__Note__: Mouse scrolling requires `mouse: true` on either the screen or the
widget. Keyboard scrolling requires `keys: true` (and optionally `vi: true`).
...
- __mouse__ - Whether to automatically enable mouse support for this list
  (allows clicking items and mouse wheel scrolling).
...
- __multiSelect__ - Enable multi-selection mode. When enabled, use space to
  toggle selection on items, click to toggle, and enter to confirm. The `select`
  event will emit an array of selected items instead of a single item.
...
- Inherits all from Box.
- __select__ - Received when an item is selected. In multiSelect mode, receives
  an array of items and array of indices.
...
- __toggle select__ - (multiSelect only) Emitted when an item's selection is
  toggled. Receives (item, index, isSelected).
...
- __fuzzyFind([string/regex/callback])__ - Find an item based on its text
  content.
- __toggleSelect(index)__ - Toggle selection of an item at index (multiSelect
  mode only).
- __selectAll()__ - Select all items in the list (multiSelect mode only).
- __clearSelection()__ - Clear all selections (multiSelect mode only).
- __getSelected()__ - Returns selected item(s). In multiSelect mode, returns an
  array of selected items; otherwise returns the single selected item.
- __getSelectedIndices()__ - Returns array of selected indices.
- __getSelectedValues()__ - Returns array of selected values (text content).
...
- __hint__ - Placeholder text to display when the input is empty (displayed in
  gray).
...
##### Keyboard Navigation (in input mode):

When in input mode (after pressing `i`, `enter`, or when `inputOnFocus` is set),
the following keyboard shortcuts are available:

- __Arrow keys__ - Move cursor left/right/up/down through the text.
- __Home__ - Move cursor to the beginning of the current line.
- __End__ - Move cursor to the end of the current line.
- __Backspace__ - Delete character before cursor.
- __Delete__ - Delete character at cursor (forward delete).
- __Escape__ - Exit input mode.
- __Ctrl+e__ - Open external editor (when `keys` option is enabled).

The cursor navigation supports Unicode characters including CJK (Chinese,
Japanese, Korean) double-width characters and surrogate pairs.

**Note:** For editing large text, consider using the external editor (`e` key)
as cursor navigation has a performance cost proportional to cursor position.
...
If you experience issues with mouse events (such as garbage characters appearing
when moving the mouse, especially with nodemon or other process managers), ensure
your environment is properly configured:

``` bash
# Set proper locale and terminal type
export LANG=en_US.utf8
export TERM=xterm-256color

# With nodemon, use -I flag and set environment
LANG=en_US.utf8 TERM=xterm-256color nodemon -I your-app.js
```

Common causes of mouse-related display corruption:
- Missing or incorrect `LANG` environment variable
- Incorrect `TERM` environment variable
- Process managers (nodemon, pm2) not properly forwarding terminal settings
...
3. Does Terminal.app support mouse events?
  - Yes. Terminal.app has supported mouse events since macOS 10.11 (El Capitan).
    Ensure mouse reporting is enabled in your blessed application.
