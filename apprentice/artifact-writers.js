...
/**
 * Write a requirement artifact — a feature requirement generated by
 * the Requirements Analyst when persistent failure indicates a
 * capability gap in the CliUI library.
 *
 * Adds classification (capability_gap, code_quality, prompt_clarity)
 * and episodeId to front-matter for traceability and filtering.
 *
 * @param {object} opts — { title, tags, confidence, body, source, classification, episodeId }
 * @returns {Promise<{id: string, path: string}>}
 */
async function writeRequirementArtifact(opts) {
    return writeArtifact("requirement", CONFIG.paths.requirements, {
        ...opts,
        extra: {
            classification: opts.classification || "unknown",
            episodeId: opts.episodeId || "unknown",
        },
    });
}
